...
pub struct BlockOwnership {
    ...
    block_to_node: Arc<DashMap<u32, String>>,

    // Node ID → Vec<Block IDs> mapping
    node_to_blocks: Arc<DashMap<String, Vec<u32>>>,

    // When ownership was assigned (for migration decisions)
    ownership_timestamps: Arc<DashMap<u32, Instant>>,
}
...
impl BlockOwnership {
    ...
    pub fn register_block(&self, block_id: u32, node_id: String) -> Result<()> {
    ...
    pub fn unregister_block(&self, block_id: u32) -> Result<()> {
    ...
    pub fn blocks_owned_by(&self, node_id: &str) -> Vec<u32> {
        self.node_to_blocks
            .get(node_id)
            .map(|entry| entry.value().clone())
            .unwrap_or_default()
    ...
    /// Get age of block ownership (time since registered)
    pub fn ownership_age(&self, block_id: u32) -> Option<std::time::Duration> {
        self.ownership_timestamps.get(&block_id).map(|entry| {
    ...
    pub fn is_owned_by(&self, block_id: u32, node_id: &str) -> bool {
        self.block_to_node
            .get(&block_id)
    ...
    /// Total number of owning nodes
    pub fn total_owners(&self) -> usize {
        self.node_to_blocks.len()
    }
}
...
mod tests {
    ...
    fn test_migrate_blocks() {
        ...
        assert_eq!(ownership.owner_of(2).unwrap(), "node-2");
        assert_eq!(ownership.blocks_owned_by("node-1").len(), 0);
        assert_eq!(ownership.blocks_owned_by("node-2").len(), 2);
    }
    ...
    fn test_unregister_block() {
        ...
        ownership.unregister_block(1).unwrap();

        assert!(ownership.owner_of(1).is_err());
    }
    ...
    fn test_is_owned_by() {
        ...
        assert!(ownership.is_owned_by(1, "node-1"));
        assert!(!ownership.is_owned_by(1, "node-2"));
    }
}
